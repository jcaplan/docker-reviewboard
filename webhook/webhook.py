...
def handle_request_publish(data):
    ...
    jen = jenkins.Jenkins('http://jenkins:8080', username='admin', password='admin')
    jen.build_job('svn-demo', {'review.url': url})


def handle_review_publish(data):
    if (data['review']['links']['user']['title'] == 'admin' and
...
class Worker(Process):
    def run(self):
        while True:
            ...
            client = RBClient('http://reviewboard:8000', api_token='f2c55a7ad5e427063cf51dcdd1689b835b2856ae')
            root = client.get_root()
            review = root.get_review_request(review_request_id=id)
            comments = review.get_reviews(max_results=200)

            for c in comments:
                if c.links.user.title == 'admin' and c.ship_it:
